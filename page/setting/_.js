    ...
    urls.forEach((url, _index) => {
        ...
        chrome.permissions.contains({
			...
			$(".item-url", item)[0].addEventListener("change", e => {
				...
				chrome.storage.local.set({setting_urls: urls});
				chrome.permissions.contains({origins: [e.target.value]}, _act => {
...
document.body.addEventListener("click", e => {
    ...
    if (name === "btn_sites_delete") {
		...
		chrome.storage.local.set({setting_searchpage_upgrade: $("[name=btn_searchpage_upgrade]")[0].checked});
