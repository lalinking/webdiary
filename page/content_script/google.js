...
$("#search div.g").forEach(g => {
	...
	nod.appendChild(createNode(`<div class="wd_search_res_upgrade_tool" data-key="${key}" data-gpname="${groupName}">${groupHideBtn}${groupHideNoBtn}${groupIncognitoBtn}${groupIncognitoNoBtn}${groupRemarkBtn}${groupDeleteBtn}</div>`));
...
const setSiteRemark = (key, groupName, groupDiv) => {
	if ($(".wd_search_res_upgrade_remarkeditor", groupDiv).length > 0) {return;}
